...
Access to Aluna systems and application is limited for all users, including but not limited to workforce members, volunteers, business associates, contracted providers, and consultants. Access by any other entity is allowable only on a minimum necessary basis. All users are responsible for reporting an incident of unauthorized user or access of the organization's information systems. These safeguards have been established to address the HIPAA Security regulations including the following:
...
5. Any Aluna workforce member can request change of access using the process outlined in [§7.2 paragraph 1](#7.2-access-establishment-and-modification).
...
8. In the case of non-personal information, such as generic educational content, identification and authentication may not be required. This is the responsibility of Aluna Customers to define, and not Aluna.
...
11. Generic accounts are not allowed on Aluna systems.
...
3. Aluna maintains a minimum necessary approach to access to Customer data. As such, Aluna, including all workforce members, does not readily have access to any ePHI.
...
2. Aluna utilizes hardware and software firewalls to segment data, prevent unauthorized access, and monitor traffic for denial of service attacks.
...
3. All Customer support desk interactions must be verified before Aluna support personnel will satisfy any request having information security implications.
   * Aluna's current support desk software, Zendesk, requires users to authenticate before submitting support tickets.
   * Support issues submitted via Aluna's dashboard require that users authenticate with their Aluna account before submitting support tickets.
   * Support issues submitted by email must be verified by Aluna personnel using a phone number that has been registered with the corresponding account.
...
All workstations at Aluna are company owned, and all are laptop Apple products running Mac OSX or Linux.
...
4. Wireless networks managed within Aluna non-production facilities (offices, etc.) are secured with the following configurations:
   * All data in transit over wireless is encrypted using WPA2 encryption;
   * Passwords are rotated on a regular basis, presently quarterly. This process is managed by the Aluna Security Officer.
...
Aluna does not use paper records for any sensitive information. Use of paper for recording and storing sensitive data is against Aluna policies.
...
12. In cases where a user has forgotten their password, the following procedure is used to reset the password.
    * The user submits a password reset request to password-reset@aluna.io. The request should include the system to which the user has lost access and needs the password reset.
...
2. Disallowing transfer of ePHI to workstations is enforced through technical measures.
   * All production access to systems is performed through a bastion/jump host accessed through a VPN. Direct access to production systems is disallowed by Aluna's VPN configuration.
   ...
   * Configuration settings for enforcing these technical controls are managed by Aluna's configuration management tooling, Chef/Salt.

## 7.14 PaaS Customer Access to Systems

Aluna grants PaaS customer secure system access via VPN connections. This access is only to Customer-specific systems, no other systems in the environment. These connections are setup at customer deployment. These connections are secured and encrypted and the only method for customers to connect to Aluna hosted systems.

In the case of data migration, Aluna does, on a case by case basis, support customers in importing data. In these cases Aluna requires that all data is secured and encrypted in transit, such as by using SFTP or SCP for transferring files.

In the case of an investigation, Aluna will assist customers, at Aluna's discretion, and law enforcement in forensics.
